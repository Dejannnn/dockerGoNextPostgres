...
const UserInterface: React.FC<UserInterfaceProps> = ({ backendName }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = (await fetch(`${apiUrl}/api/users`)).json();
        setUsers((await response).reverse());
      } catch (error) {
  ...
  const createUser = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = (await fetch(`${apiUrl}/api/users`, { method: "POST", body: JSON.stringify(newUser)})).json();
      setUsers([await response, ...users]);
  ...
  const handleUpdateUser = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      await fetch(`${apiUrl}/api/users/${updateUser.id}`, {method:"PUT", body: JSON.stringify({name: updateUser.name, email: updateUser.email}) });
  ...
  return (
    <div className={`user-interface ${bgColor} ${backendName} w-full max-w-md p-4 my-4 rounded shadow`}>
      <img src={`/next.svg`} alt={`${backendName} Logo`} className="w-20 h-20 mb-6 mx-auto" />
      ...
      <form onSubmit={createUser} className="mb-6 p-4 bg-blue-100 rounded shadow">
        ...
        <input
          ...
          className="mb-2 w-full p-2 border text-black border-gray-300 rounded"
        />
